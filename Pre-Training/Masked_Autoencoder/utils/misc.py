...
def save_checkpoint_for_finetune(save_to, args, sp_cnn_state, epoch, last_loss): # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if epoch % 50 == 0: # Nur alle 50 Epochen einen Checkpoint speichern
        epoch_number = 'Epoch_%s_' % epoch # Schreibt die Nummer dazu
        loss = "Loss_%s_" % last_loss
        checkpoint_path = os.path.join(args.exp_dir, epoch_number + loss + save_to)
        if dist.is_local_master():
            to_save = {
                'arch': args.model,
                'module': sp_cnn_state,
