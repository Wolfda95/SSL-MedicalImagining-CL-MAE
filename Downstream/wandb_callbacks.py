# From Lightning-Hydra (https://github.com/ashleve/lightning-hydra-template)
...
# class LogF1AUCTest(Callback):
#     """Generate ROCAuC for test run.
#     Expects test step to return lable and class.
#     """
#
#     def __init__(self, class_names: List[str] = None):
#         self.preds = []
...
#         preds = torch.cat(self.preds).cpu().numpy()
#         targets = torch.cat(self.targets).cpu().numpy()
#         outputs = torch.cat(self.outputs).cpu().numpy()
#
#         f1 = f1_score(targets, preds, average=None)
#         r = recall_score(targets, preds, average=None)
#         p = precision_score(targets, preds, average=None)
...
#
#         # names should be uniqe or else charts from different experiments in wandb will overlap
#         experiment.log({f"test_f1/{experiment.name}": wandb.Image(plt)}, commit=False)
#         experiment.log({f"test_roc" : wandb.plot.roc_curve(targets.tolist(), np.eye(2)[outputs].tolist())})
#
#         # reset plot
#         plt.clf()
#
